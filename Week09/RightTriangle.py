...
def rightTriangle2(N):
    ...
    for i in range(1, N):
        for j in range(i, N + 1):
            ...
            if i == j:
                case2 += (math.floor((min(math.floor(c), N) - j )) + math.floor((min(math.floor(d), N) - i)))
            else:
                gcd_i_j = gcd(j, i)
                case2 += 2*(math.floor((min(math.floor(c), N) - j ) / (i / gcd_i_j)) + math.floor((min(math.floor(d), N) - i) / (j / gcd_i_j)))
