...
def monopoly(numSides, numIterations):
    ...
    # 초기확률 초기화
    prob_cur = [0 for _ in range(len(squares))]
    prob_cur[0] = 1

    # 전이확률 초기화
    prob_transition = [[0 for _ in range(len(squares))] for _ in range(len(squares))]

    for i in range(len(squares)):
        for d1 in range(1, numSides + 1):
            for d2 in range(1, numSides + 1):
                sx = i
                sy = (i + d1 + d2) % 40

                prob_transition[sx][sy] += 1 / (numSides * numSides)
    ...
    for sx in range(len(squares)):
        for sy in range(len(squares)):
            ...
            # 특수 이동 (도착지가 GOTOJAIL)
            if squares[sy] == 'G2J':
                prob_transition[sx][10] += prob_transition[sx][sy]   # JAIL로 이동
                prob_transition[sx][sy] = 0
            
            # 특수 이동 (도착지가 CC)
            elif "CC" in squares[sy]:
                prob_transition[sx][0] += prob_transition[sx][sy] * (1/16)   # GO로 이동
                prob_transition[sx][10] += prob_transition[sx][sy] * (1/16)  # JAIL로 이동
                prob_transition[sx][sy] = prob_transition[sx][sy] * (14/16) # 제자리 유지

            # 특수 이동 (도착지가 CH)
            elif "CH" in squares[sy]:
                prob_transition[sx][0] += prob_transition[sx][sy] * (1/16)   # GO로 이동
                prob_transition[sx][10] += prob_transition[sx][sy] * (1/16)  # JAIL로 이동
                prob_transition[sx][11] += prob_transition[sx][sy] * (1/16)  # C1로 이동
                prob_transition[sx][24] += prob_transition[sx][sy] * (1/16)  # E3로 이동
                prob_transition[sx][39] += prob_transition[sx][sy] * (1/16)  # H2로 이동
                prob_transition[sx][5] += prob_transition[sx][sy] * (1/16)  # R1로 이동

                if squares[sy] == "CH1":
                    prob_transition[sx][15] += prob_transition[sx][sy] * (2/16)   # R2로 이동
                    prob_transition[sx][12] += prob_transition[sx][sy] * (1/16)   # U1로 이동
                    prob_transition[sx][4] += prob_transition[sx][sy] * (1/16)    # T1로 이동

                elif squares[sy] == "CH2":
                    prob_transition[sx][25] += prob_transition[sx][sy] * (2/16)   # R3로 이동
                    prob_transition[sx][28] += prob_transition[sx][sy] * (1/16)   # U2로 이동
                    prob_transition[sx][19] += prob_transition[sx][sy] * (1/16)    # D3로 이동

                else:
                    prob_transition[sx][5] += prob_transition[sx][sy] * (2/16)   # R1로 이동
                    prob_transition[sx][12] += prob_transition[sx][sy] * (1/16)   # U1로 이동
                    prob_transition[sx][0] += prob_transition[sx][sy] * (1/16) * (1/16) #GO로 이동
                    prob_transition[sx][10] += prob_transition[sx][sy] * (1/16) * (1/16) #JAIL로 이동
                    prob_transition[sx][33] += prob_transition[sx][sy] * (1/16) * (14/16)    #CC3 이동(유지)

                prob_transition[sx][sy] = prob_transition[sx][sy] * (6/16)  # 제자리(CH3) 유지

    # 반복
    for i in range(numIterations):
        prob_next = [0 for _ in range(len(squares))]
        for sx in range(len(squares)):
            for sy in range(len(squares)):
                prob_next[sy] += prob_transition[sx][sy]*prob_cur[sx]

        for j in range(len(squares)):
            prob_cur[j] = prob_next[j]

    result = []
    
    for i in range(len(squares)):
        result.append((squares[i], prob_cur[i]))
