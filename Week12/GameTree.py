...
class PTreePlayer(Player):
        ...
        while (self.root[2][0] + self.root[2][1]) < self.maxNumSimulations:
            ...
            while True:
                ...
                for child in node[3]:
                    ...
                    if child[2][0] == 0 and child[2][1] == 0:
                        cand_node = child
                        cand_val = self.CONST_INF

                    else:
                        cand_node = child
                        cand_val = child[2][self.player] / (child[2][0] + child[2][1]) + self.CONST_C*math.sqrt((node[2][0] + node[2][1])/(child[2][0] + child[2][1]))
                ...
                if max_node[2][0] == 0 and max_node[2][1] == 0:
                    break
                else:
                    node = max_node

            winPlayer = runBWGame(max_node[1][0], max_node[1][1], RandomPlayer, RandomPlayer, False)

            reverse_node = max_node

            if winPlayer == max_node[0]:
                while reverse_node != None:
                    reverse_node[2][0] += 1
                    reverse_node = reverse_node[4]
    ...
    def doMyTurn(self): 
        ...
        for child in self.root[3]:
            if child != None:
                if maxWinRate == None: 
                    maxWinRate, childWithMaxWinRate = child[2][self.player]/sum(child[2]), child
                else:
...
if __name__ == "__main__":    
    ...
    PlayerClass0, PlayerClass1 = RandomPlayer, RandomPlayer
    numGames = 10
    numWins = [0, 0]    
    for i in range(numGames):
        numWins[runBWGame(numBlack, numWhite, PlayerClass0, PlayerClass1, True)] += 1
