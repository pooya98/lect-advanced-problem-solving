...
def findProductSum(n):
    ...
    def recur(min, product, sum, depth):
        ...
        while (product * i) <= n:
            ...
            if depth >= 2:
                result.append(createProductSumSequence(element_list, depth - 1))
...
def findMinimalProductSum(n):
    mps_table = [-1 for _ in range(n + 1)]
    n_square = n * n

    def recur(min, product, sum, depth):
        i = min
        while (product * i) <= n_square:

            if depth >= 2:
                k = depth + ((product * i) - (sum + i))

                if k <= n:
                    if(mps_table[k] == -1) or (mps_table[k] > product * i) :
                        mps_table[k] = product * i
