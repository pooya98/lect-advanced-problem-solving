...
def totientMaximum3(*Ns):
    ...
    while cumul_value <= maxN:
        cumul_value *= primes[prime_index]
        products.append(cumul_value)
        prime_index += 1
...
def totientMinimum(*Ns):    
    maxN = max(Ns)
    primes = findPrimes(maxN)
    results = []

    for n in Ns:
        results.append(primes[binarySearch(primes, 0, len(primes) - 1, n)])
